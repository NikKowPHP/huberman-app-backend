...
*   `[x]` **FIX.DTO.ODS.1: Create DTOs:** `sync-offline-data.dto.ts` (based on `SyncOfflineDataRequest.php`)
...
*   `[x]` **FIX.MM.PS.2: Create `post.controller.ts`** (Endpoints: `GET /posts`, `POST /posts`, `POST /posts/{id}/comments`)
*   `[x]` **FIX.MM.PS.3: Create DTOs:** `create-post.dto.ts`, `store-comment.dto.ts`
*   `[x]` **FIX.MM.PS.4: Create `post.module.ts`**
*   `[x]` **FIX.MM.PS.5: Register `PostModule` in `app.module.ts`**
...
*   `[x]` **FIX.MM.RS.3: Create DTOs:** `store-routine.dto.ts`, `update-routine.dto.ts`
*   `[x]` **FIX.MM.RS.4: Create `routine.module.ts`**
*   `[x]` **FIX.MM.RS.5: Register `RoutineModule` in `app.module.ts`**
...
*   **(Implement the following new services using the pattern above, porting logic from their Laravel counterparts):**
    *   `[x]` **FIX.BL.TS.1: `TrackingService` - Implement** (from `app/Modules/TrackingService/Services/TrackingService.php`)
    *   `[x]` **FIX.BL.ODS.1: `OfflineDataService` - Implement** (from `app/Services/OfflineDataService.php`)
    *   `[x]` **FIX.BL.PS.1: `PostService` - Implement** (from `app/Services/PostService.php`)
    *   `[x]` **FIX.BL.RSVC.1: `RoutineService` - Implement** (from `app/Services/RoutineService.php`)
...
*   **(Repeat FIX.DTO.*.1 and FIX.CTRL.*.1 for other new/stubbed modules):**
    *   `[x]` **FIX.DTO.ODS.1 & FIX.CTRL.ODS.1:** `OfflineDataController` & `sync-offline-data.dto.ts`
...
*   `[x]` **FIX.NOTIF.S.1: `NotificationService` - Implement Real FCM/APNS Sending**
